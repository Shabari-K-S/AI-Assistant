...
export const TelemetryGauges = memo(function TelemetryGauges({ snap, connected }: Props) {
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 w-full">
      ...
      <div className="hud-panel p-2.5 rounded flex flex-col justify-between">
        <div className="flex items-center justify-between">
          <span className="hud-label text-[9px]">Ambient RMS</span>
          <span className="font-mono text-[9px] text-[#7da4b8]">LIVE</span>
        </div>
        <div className="mt-2 flex items-baseline justify-between">
          <span className="font-mono text-lg font-bold text-[#e8fbff]">
            {snap.noise_floor.toFixed(3)}
          </span>
          <span className="font-mono text-[10px] text-[#7da4b8]">RMS</span>
        </div>
        <div className="w-full bg-[rgba(65,230,255,0.1)] h-1 rounded mt-1.5 overflow-hidden">
          <div
            className="h-full bg-[#7ef3ff] transition-all duration-300"
            style={{ width: `${Math.min(100, Math.round(snap.noise_floor * 1000))}%` }}
          />
        </div>
      </div>

      {/* Gauge 3: System Engine */}
      <div className="hud-panel p-2.5 rounded flex flex-col justify-between">
        <div className="flex items-center justify-between">
